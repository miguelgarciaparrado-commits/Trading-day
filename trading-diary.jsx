...
function AlertasTab({S,predictions}){
  ...
  function sendAlert(label,interval,rsi,type,e1,e2,customDesc,price,extra){
    ...
    if(tk&&cid){
      ...
      // FVG más lejano — escanear TODAS las temporalidades y elegir el más lejano
      var fvgFarExit=null,fvgFarDist=0;
      var fvgTfLabel="";
      var allFvgSrc=ohlcData.length>4?[{tf:interval,ohlc:ohlcData}]:[];
      ["1h","4h","1d","1w"].forEach(function(tfk2){
        if(ohlcByTf[tfk2].length>4&&tfk2!==interval)allFvgSrc.push({tf:tfk2,ohlc:ohlcByTf[tfk2]});
      });
      ...
      }else if(hasMultiTfConf){
        tgLines.push("💡 OPERACIÓN ("+sugDir+") — "+crossTfCount+" TFs alineadas:");
        if(fbLowPerf){
  ...
  function startStockAlertFinnhub(alert){
    ...
    function processStockCandles(d,isLive){
      ...
      if(divResultFH){
        ...
        var divCooldownFH=divKindFH==="convergence"?baseCdFH*2:baseCdFH;
          ...
          if(rsi!==null){
            ...
            if(alert.rsiDivEnabled!==false){
              if(divResult){
                ...
                var divCooldownMs=divKind==="convergence"?baseCdMs*2:baseCdMs;
