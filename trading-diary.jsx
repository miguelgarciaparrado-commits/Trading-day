          ...
          <div>
            ...
            <div style={S.card}>
              <div style={{maxHeight:500,overflowY:"auto"}}>
                {(function(){
                  // Detect mergeable groups: same asset+dir+cap+date, note contains "TP parcial"
                  var mergeGroups={};
                  fH.forEach(function(h){
                    var isApp=xhist.some(function(x){return x.id===h.id;});
                    if(!isApp)return;
                    var note=h.note||"";
                    if(note.indexOf("TP parcial")<0&&note.indexOf("parcial")<0)return;
                    var key=h.asset+"_"+h.dir+"_"+h.cap+"_"+(h.date||"");
                    if(!mergeGroups[key])mergeGroups[key]=[];
                    mergeGroups[key].push(h.id);
                  });
                  // Only keep groups with 2+ entries
                  Object.keys(mergeGroups).forEach(function(k){if(mergeGroups[k].length<2)delete mergeGroups[k];});
                  // Build a map: id → groupKey
                  var idToGroup={};
                  Object.keys(mergeGroups).forEach(function(k){
                    mergeGroups[k].forEach(function(id){idToGroup[id]=k;});
                  });
                  // Track which groups have already shown the merge button
                  var shownMerge={};

                  return fH.map(function(h){
                    var isAppEntry=xhist.some(function(x){return x.id===h.id;});
                    var groupKey=idToGroup[h.id]||null;
                    var isLastInGroup=groupKey&&mergeGroups[groupKey]&&mergeGroups[groupKey][mergeGroups[groupKey].length-1]===h.id;
                    return(
                      <div key={h.id} style={{...S.row,background:groupKey?"rgba(240,180,41,.03)":"transparent"}}>
                        <div style={{display:"flex",gap:7,alignItems:"center",flex:1,minWidth:0}}>
                          ...
                          {h.note&&<span style={{...S.bdg(h.note.includes("LIQUIDACION")?"#ff6600":groupKey?"rgba(240,180,41,.6)":"#555"),maxWidth:120,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{h.note}</span>}
                        </div>
                        <div style={{display:"flex",gap:6,alignItems:"center",flexShrink:0}}>
                          ...
                          {isLastInGroup&&(function(){
                            var ids=mergeGroups[groupKey];
                            return(
                              <button onClick={function(){
                                var toMerge=D.current.xhist.filter(function(x){return ids.indexOf(x.id)>=0;});
                                var totalResult=parseFloat(toMerge.reduce(function(a,x){return a+(x.result||0);},0).toFixed(2));
                                var merged={id:Date.now(),asset:h.asset,dir:h.dir,cap:h.cap,
                                  result:totalResult,date:h.date||"",
                                  note:"TPs completados ("+toMerge.length+" niveles)"};
                                var remaining=D.current.xhist.filter(function(x){return ids.indexOf(x.id)<0;});
                                SX([merged,...remaining]);
                              }} style={{background:"rgba(240,180,41,.15)",border:"1px solid rgba(240,180,41,.4)",color:"#f0b429",fontSize:8,cursor:"pointer",padding:"2px 7px",borderRadius:3,fontWeight:700,whiteSpace:"nowrap"}}>
                                Combinar {mergeGroups[groupKey].length}
                              </button>
                            );
                          })()}
                          {isAppEntry&&!groupKey&&<button onClick={function(){SX(D.current.xhist.filter(function(x){return x.id!==h.id;}));}} style={{background:"rgba(255,68,68,.12)",border:"1px solid rgba(255,68,68,.3)",color:"#ff6666",fontSize:11,cursor:"pointer",padding:"2px 6px",borderRadius:3,lineHeight:1,fontWeight:700}} title="Eliminar entrada">×</button>}
                          {isAppEntry&&groupKey&&!isLastInGroup&&<button onClick={function(){SX(D.current.xhist.filter(function(x){return x.id!==h.id;}));}} style={{background:"transparent",border:"none",color:"#333",fontSize:11,cursor:"pointer",padding:"2px 4px",lineHeight:1}} title="Eliminar">×</button>}
                        </div>
                      </div>
                    );
                  });
...
function AlertasTab({S}){
  ...
  function sendAlert(label,interval,rsi,type,e1,e2,customDesc,price,extra){
    ...
    if(tk&&cid){
      ...
      // Banderín alcista: TP = altura del asta proyectada desde la ruptura
      if(!tpPrice&&extra.pennantResult&&extra.pennantResult.targetPrice){
        tpPrice=extra.pennantResult.targetPrice;tpNote="Objetivo banderín (altura asta)";
      }
      ...
      if(extra.pennantResult){
        ...
        if(type==="pennant_fake"&&pr2.falseBrkReasons&&pr2.falseBrkReasons.length>0){
          ...
          if(k.x){
            ...
            if(pennantResult&&pennantResult.breakout){
              if(!pennantResult.falseBrkWarning){
                // Confirmed breakout — fire once per price level
                ...
                // Possible false breakout — warn once
  ...
  pos.forEach(p=>{
    ...
    if(ratio&&ratio>20){
      tips.push({
        type:"warn",
        icon:"⚠",
        text:"Ratio "+ratio.toFixed(0)+":1 es muy alto. Revisa si el TP es realista. Considera tomar beneficio parcial en niveles intermedios."
      });
    }
