...
function AlertasTab({S,predictions}){
  ...
  function sendAlert(label,interval,rsi,type,e1,e2,customDesc,price,extra){
    ...
    if(tk&&cid){
      ...
      var BULLISH_TYPES=["rsi_oversold","golden","ema200_golden","rsi_div_bull","rsi_conv_bull","patron_fvg","canal_bajista_ruptura_alcista","canal_alcista_soporte","canal_alcista_ruptura_alcista"];
      var BEARISH_TYPES=["rsi_overbought","death","ema200_death","rsi_div_bear","rsi_conv_bear","canal_alcista_ruptura_bajista","canal_bajista_resistencia","canal_bajista_ruptura_bajista","canal_alcista_resistencia"];
  ...
  function startAlert(alert){
    ...
    // Reset channel state — channel type uses localStorage cooldown to prevent reconnect re-fires
    ...
    function doFetchKlines(attempt){
      ...
      .then(function(data){
        ...
        ws.onmessage=function(e){
          ...
          // ─── PATRONES CHARTISTAS (SOLO CANALES) ───
          if(k.x){
            ...
            if(!chanResult){
              ...
              if(curCT!==prevCT){
                // ─── Nuevo canal (incluye primera detección tras inicio, prevCT=null) ───
                lastTrigRef.current[ak+"chan_was_out"]=false;
                lastTrigRef.current[ak+"chan_hp_done"]=false;
                ALL_CHAN_KEYS.forEach(function(ct){delete confluenceRef.current[confKeyPat][ct];});
                // localStorage cooldown prevents re-notification on WS reconnect
                var chanLsKey="td-chan-"+ak+curCT;
                var chanLsTs=0;
                try{chanLsTs=parseInt(localStorage.getItem(chanLsKey)||"0");}catch(e){}
                var chanRecentNotif=Date.now()-chanLsTs<chanIntervalMs*3;
                if(!chanResult.falseBrk&&!chanRecentNotif){
                  var newNotifType;
                  ...
                  try{localStorage.setItem(chanLsKey,Date.now().toString());}catch(e){}
                }
