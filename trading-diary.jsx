  ...
  function sendAlert(label,interval,rsi,type,e1,e2,customDesc,price,extra){
    ...
    if(tk&&cid){
      ...
      if((type==="golden"||type==="death")&&e1&&e1>0){
        ...
        // Rebote en resistencia del canal bajista: entrada en la línea de resistencia
        ...
        // Ruptura alcista: entrada en la resistencia rota (posible retest)
        entryPrice=parseFloat(parseFloat(extra.channelResult.topLine).toFixed(2));
        ...
        // Ruptura bajista: entrada en el soporte roto (posible retest)
        entryPrice=parseFloat(parseFloat(extra.channelResult.botLine).toFixed(2));
        ...
        // Banderín alcista: entrada en la línea de resistencia del banderín (punto de ruptura)
        ...
        // Banderín bajista: entrada en el soporte del banderín roto
        entryPrice=parseFloat(extra.pennantResult.lowerLine.toFixed(2));
        entryNote="Ruptura bajista banderín";
      }
      // RSI, divergencias, FVG: precio de mercado actual (no hay nivel técnico derivado)
      var slPct=0.02;
      ...
      if(ohlcData.length>4&&riskAmt>0){
        ...
        // Ordenar por proximidad al precio de entrada
        fvgCands.sort(function(a,b){return Math.abs(a.price-entryPrice)-Math.abs(b.price-entryPrice);});
        // Usar el FVG más cercano que dé ratio ≥1:2
        for(var ci=0;ci<fvgCands.length;ci++){
          var cand=fvgCands[ci];
          var candReward=Math.abs(cand.price-entryPrice);
          if(candReward/riskAmt>=2.0){tpPrice=cand.price;tpNote=cand.note;break;}
        }
      }
      // Banderín: TP = altura del asta proyectada (si da ≥1:2)
      if(!tpPrice&&extra.pennantResult&&extra.pennantResult.targetPrice){
        var pennTP=extra.pennantResult.targetPrice;
        var pennReward=Math.abs(pennTP-entryPrice);
        if(riskAmt<=0||pennReward/riskAmt>=2.0){
          tpPrice=pennTP;
