  ...
  useEffect(function(){
    ...
    var iv2=setInterval(function(){
      ...
      var LABELS={
        ...
        pennant_bull:"🚩 Banderín Alcista",pennant_bull_forming:"🚩 Banderín Alcista (Formando)",pennant_fake:"⚠️ Banderín Alcista Falso",
        pennant_bear:"🔻 Banderín Bajista",pennant_bear_forming:"🔻 Banderín Bajista (Formando)",pennant_bear_fake:"⚠️ Banderín Bajista Falso",
...
function ChatTab({S,pos,PM,pats,ps,sc,jnl,hist,xhist,SPs,SJ,D,save,predictions,SPred,initialChatMsgs}){
  ...
  function buildSystemPrompt(md,withImage){
    ...
    if(md){
      ...
      var pennantText="No detectado";
      if(md.pennantResult){
        var pn=md.pennantResult;
        if(pn.breakout&&!pn.falseBrkWarning){
          pennantText="🚩 BANDERÍN ALCISTA CONFIRMADO — ruptura válida | Asta +"+( pn.poleGainPct*100).toFixed(1)+"% | Consolidación "+pn.consLength+" velas | Objetivo $"+pn.targetPrice.toFixed(2);
        }else if(pn.breakout&&pn.falseBrkWarning){
          pennantText="⚠️ Banderín alcista con posible FALSA RUPTURA ("+pn.falseBrkReasons.join(", ")+") | Objetivo tentativo $"+pn.targetPrice.toFixed(2);
        }else{
          pennantText="Banderín alcista en formación (aún no hay ruptura) — asta +"+( pn.poleGainPct*100).toFixed(1)+"% | Resistencia superior $"+pn.upperLine.toFixed(2)+" | Objetivo potencial $"+pn.targetPrice.toFixed(2);
        }
      }
...
function AlertasTab({S,predictions}){
  ...
  function sendAlert(label,interval,rsi,type,e1,e2,customDesc,price,extra){
    ...
    else if(type==="pennant_bull_forming"){title="🚩 Banderín Alcista — En Formación";body="Triángulo convergente detectado. Vigilar ruptura al alza."+priceStr;}
    ...
    else if(type==="pennant_bear_forming"){title="🔻 Banderín Bajista — En Formación";body="Triángulo convergente bajista detectado. Vigilar ruptura a la baja."+priceStr;}
    else if(type==="pennant_bear_fake"){title="⚠️ Banderín Bajista — Posible Falsa Ruptura";}
    ...
    else if(type==="canal_bajista_ruptura_alcista"){title="🚀 Canal Bajista — Ruptura Alcista";body=(customDesc||"")+priceStr;}
    else if(type==="canal_bajista_ruptura_bajista"){title="📐 Canal Bajista — Ruptura Bajista Confirmada";body=(customDesc||"")+priceStr;}
    else if(type==="canal_alcista_soporte"){title="📐 Canal Alcista — Precio en Soporte";body=(customDesc||"")+priceStr;}
    else if(type==="canal_alcista_resistencia"){title="📐 Canal Alcista — Precio en Resistencia";body=(customDesc||"")+priceStr;}
    else if(type==="canal_alcista_ruptura_alcista"){title="🚀 Canal Alcista — Ruptura Alcista Confirmada";body=(customDesc||"")+priceStr;}
    else if(type==="canal_alcista_ruptura_bajista"){title="⚠️ Canal Alcista — Ruptura Bajista";body=(customDesc||"")+priceStr;}
    ...
    if(tk&&cid){
      ...
      var BULLISH_TYPES=["rsi_oversold","golden","ema200_golden","rsi_div_bull","rsi_conv_bull","patron_fvg","pennant_bull","pennant_bull_forming","pennant_fake","canal_bajista_ruptura_alcista","canal_alcista_soporte","canal_alcista_ruptura_alcista"];
      var BEARISH_TYPES=["rsi_overbought","death","ema200_death","rsi_div_bear","rsi_conv_bear","pennant_bear","pennant_bear_forming","pennant_bear_fake","canal_alcista_ruptura_bajista","canal_bajista_resistencia","canal_bajista_ruptura_bajista","canal_alcista_resistencia"];
      ...
      // ─── PRECIO DE ENTRADA según estrategia ───
      var entryPrice=price||0;
      var entryNote="";
      if((type==="golden"||type==="death")&&e1&&e1>0){
      ...
      }else if((type==="pennant_bull"||type==="pennant_fake")&&extra.pennantResult&&extra.pennantResult.upperLine){
        entryPrice=parseFloat(extra.pennantResult.upperLine.toFixed(2));
        entryNote="Ruptura banderín";
      }else if((type==="pennant_bear"||type==="pennant_bear_fake")&&extra.pennantResult&&extra.pennantResult.lowerLine){
        entryPrice=parseFloat(extra.pennantResult.lowerLine.toFixed(2));
        entryNote="Ruptura bajista banderín";
      }else if((type==="rsi_div_bull"||type==="rsi_conv_bull"||type==="rsi_oversold")&&ohlcData.length>4){
        // Divergencia/RSI alcista: buscar FVG alcista cercano (±3%) para entrada de precisión ICT
        var fvgE=findNearFVG(ohlcData,price||0,true,0.03);
        if(fvgE){entryPrice=parseFloat(fvgE.bot.toFixed(2));entryNote="FVG alcista cercano";}
      }else if((type==="rsi_div_bear"||type==="rsi_conv_bear"||type==="rsi_overbought")&&ohlcData.length>4){
        // Divergencia/RSI bajista: buscar FVG bajista cercano (±3%) para entrada de precisión ICT
        var fvgE2=findNearFVG(ohlcData,price||0,false,0.03);
        if(fvgE2){entryPrice=parseFloat(fvgE2.top.toFixed(2));entryNote="FVG bajista cercano";}
      }
      ...
      }else if((type==="pennant_bull"||type==="pennant_fake")&&extra.pennantResult&&extra.pennantResult.lowerLine){
        slPrice=parseFloat((extra.pennantResult.lowerLine*0.99).toFixed(2));
      }else if((type==="pennant_bear"||type==="pennant_bear_fake")&&extra.pennantResult&&extra.pennantResult.upperLine){
        slPrice=parseFloat((extra.pennantResult.upperLine*1.01).toFixed(2));
      ...
      // Banderín: TP = proyección del mástil
      if(!tpPrice&&extra.pennantResult&&extra.pennantResult.targetPrice){
        var pennTP=extra.pennantResult.targetPrice;
        if(riskAmt<=0||Math.abs(pennTP-entryPrice)/riskAmt>=1.5){
          tpPrice=pennTP;
      ...
      if(extra.pennantResult){
        var pr2=extra.pennantResult;
        tgLines.push("");
        var isBearPennant=(type==="pennant_bear"||type==="pennant_bear_fake"||type==="pennant_bear_forming");
        var isForming=(type==="pennant_bull_forming"||type==="pennant_bear_forming");
        tgLines.push(isBearPennant?"🔻 BANDERÍN BAJISTA:"+(isForming?" (EN FORMACIÓN)":""):"🚩 BANDERÍN ALCISTA:"+(isForming?" (EN FORMACIÓN)":""));
        if(isBearPennant){
          tgLines.push("   📏 Mástil: −"+((pr2.poleDropPct||0)*100).toFixed(1)+"% | Consolidación: "+pr2.consLength+" velas");
          tgLines.push("   📐 Triángulo: línea sup $"+pr2.upperLine.toFixed(2)+" | línea inf $"+pr2.lowerLine.toFixed(2));
          if(!isForming){tgLines.push("   🎯 Objetivo (proyección mástil): $"+pr2.targetPrice.toFixed(2));}
          if(!isForming){tgLines.push("   ⚠️ Ruptura por DEBAJO del triángulo — continuación bajista");}
          if(isForming){tgLines.push("   ⏳ Triángulo convergente bajista en formación — vigilar ruptura a la baja");}
        }else{
          tgLines.push("   📏 Asta: +"+((pr2.poleGainPct||0)*100).toFixed(1)+"% | Consolidación: "+pr2.consLength+" velas");
          tgLines.push("   📐 Triángulo: línea sup $"+pr2.upperLine.toFixed(2)+" | línea inf $"+pr2.lowerLine.toFixed(2));
          if(!isForming){tgLines.push("   🎯 Objetivo (altura asta): $"+pr2.targetPrice.toFixed(2));}
          if(isForming){tgLines.push("   ⏳ Triángulo convergente alcista en formación — vigilar ruptura al alza");}
        }
        if((type==="pennant_fake"||type==="pennant_bear_fake")&&pr2.falseBrkReasons&&pr2.falseBrkReasons.length>0){
  ...
  function startAlert(alert){
    ...
    lastTrigRef.current[ak0Init+"pennant_bull_forming"]=false;
    lastTrigRef.current[ak0Init+"pennant_fake"]=false;
    lastTrigRef.current[ak0Init+"pennant_bear"]=false;
    lastTrigRef.current[ak0Init+"pennant_bear_forming"]=false;
        ...
        ws.onmessage=function(e){
          ...
          if(k.x){
            ...
            if(lastTrigRef.current[ak+"firstClosed"]){
              ...
              lastTrigRef.current[ak+"pennant_bull_forming"]=!!(syncPennB&&syncPennB.forming);
              ...
              lastTrigRef.current[ak+"pennant_bear_forming"]=!!(syncPennBr&&syncPennBr.forming);
            ...
            // ─── Banderín alcista (tri-state: null / forming / breakout) ───
            var pennantResult=detectBullishPennant(ohlc);
            if(!pennantResult){
              // Pattern disappeared entirely
              lastTrigRef.current[ak+"pennant_bull_forming"]=false;
              lastTrigRef.current[ak+"pennant_bull"]=false;
              lastTrigRef.current[ak+"pennant_fake"]=false;
              delete confluenceRef.current[confKeyPat]["pennant_bull"];
              delete confluenceRef.current[confKeyPat]["pennant_bull_forming"];
            }else if(pennantResult.forming&&!pennantResult.breakout){
              // Pattern forming, no breakout yet — notify first time
              lastTrigRef.current[ak+"pennant_bull"]=false;
              lastTrigRef.current[ak+"pennant_fake"]=false;
              if(!lastTrigRef.current[ak+"pennant_bull_forming"]){
                sendAlert(alert.label,alert.interval,rsi,"pennant_bull_forming",ema7,ema25,null,closePrice,{ohlc:ohlc,pennantResult:pennantResult,divResult:divResult});
              }
              confluenceRef.current[confKeyPat]["pennant_bull_forming"]=Date.now();
              lastTrigRef.current[ak+"pennant_bull_forming"]=true;
            }else if(pennantResult.breakout){
              // Breakout detected
              lastTrigRef.current[ak+"pennant_bull_forming"]=false;
              delete confluenceRef.current[confKeyPat]["pennant_bull_forming"];
            ...
            // ─── Banderín bajista (tri-state: null / forming / breakout) ───
            var bearPennantResult=detectBearishPennant(ohlc);
            if(!bearPennantResult){
              lastTrigRef.current[ak+"pennant_bear_forming"]=false;
              lastTrigRef.current[ak+"pennant_bear"]=false;
              lastTrigRef.current[ak+"pennant_bear_fake"]=false;
              delete confluenceRef.current[confKeyPat]["pennant_bear"];
              delete confluenceRef.current[confKeyPat]["pennant_bear_forming"];
            }else if(bearPennantResult.forming&&!bearPennantResult.breakout){
              lastTrigRef.current[ak+"pennant_bear"]=false;
              lastTrigRef.current[ak+"pennant_bear_fake"]=false;
              if(!lastTrigRef.current[ak+"pennant_bear_forming"]){
                sendAlert(alert.label,alert.interval,rsi,"pennant_bear_forming",ema7,ema25,null,closePrice,{ohlc:ohlc,pennantResult:bearPennantResult,divResult:divResult});
              }
              confluenceRef.current[confKeyPat]["pennant_bear_forming"]=Date.now();
              lastTrigRef.current[ak+"pennant_bear_forming"]=true;
            }else if(bearPennantResult.breakout){
              lastTrigRef.current[ak+"pennant_bear_forming"]=false;
              delete confluenceRef.current[confKeyPat]["pennant_bear_forming"];
          ...
          if(k.x&&alert.interval!=="1w"){
            if(alert.emaCross725Enabled!==false){
              if(cross7_25==="golden"){
                if(!lastTrigRef.current[ak+"g725"]){lastTrigRef.current[ak+"g725"]=true;sendAlert(alert.label,alert.interval,rsi,"golden",ema7,ema25,null,closePrice,{ohlc:ohlc,divResult:divResult,ema50:ema50,ema200:ema200});}
              }else{lastTrigRef.current[ak+"g725"]=false;}
              if(cross7_25==="death"){
                if(!lastTrigRef.current[ak+"d725"]){lastTrigRef.current[ak+"d725"]=true;sendAlert(alert.label,alert.interval,rsi,"death",ema7,ema25,null,closePrice,{ohlc:ohlc,divResult:divResult,ema50:ema50,ema200:ema200});}
              }else{lastTrigRef.current[ak+"d725"]=false;}
            }
  ...
  return(
    <div>
      ...
      {(function(){
        var FB_LABELS={
          pennant_bull:"🚩 Banderín Alcista",pennant_fake:"⚠️ Banderín Falso",
      ...
      {logs.length>0&&(
        <div style={{background:"#111118",border:"1px solid #1e1e2e",borderRadius:8,padding:12,marginTop:4}}>
          ...
          {logs.slice(0,20).map(function(log){
            ...
            else if(log.type==="pennant_bull"||log.type==="pennant_bull_forming"||log.type==="pennant_fake"){icon="🚩";color="#00ff88";}
            else if(log.type==="pennant_bear"||log.type==="pennant_bear_forming"||log.type==="pennant_bear_fake"){icon="🔻";color="#ff4444";}
            ...
            var ALL_FB_TYPES=["rsi_oversold","rsi_overbought","rsi_custom","golden","death","ema200_golden","ema200_death",
              "rsi_div_bull","rsi_div_bear","rsi_conv_bull","rsi_conv_bear","pennant_bull","pennant_bull_forming","pennant_fake","pennant_bear","pennant_bear_forming","pennant_bear_fake",
...
// Structure: strong upward pole → small triangular consolidation (descending highs, ascending lows,
// decreasing volume) → bullish breakout above upper trendline with volume
function detectBullishPennant(ohlc){
  if(!ohlc||ohlc.length<20)return null;
  var best=null;
  for(var consLen=15;consLen>=5;consLen--){
    var consEnd=ohlc.length-1;
    var consStart=consEnd-consLen+1;
    if(consStart<5)continue;
    // Build regression on ALL candles EXCEPT the last one (potential breakout candle)
    // This prevents the breakout candle from distorting its own trendline
    var regSlice=ohlc.slice(consStart,consEnd); // excludes last candle
    if(regSlice.length<4)continue;
    var highPts=regSlice.map(function(c,i){return{x:i,y:c.h};});
    var lowPts=regSlice.map(function(c,i){return{x:i,y:c.l};});
    var rH=linReg(highPts);
    var rL=linReg(lowPts);
    if(!rH||!rL)continue;
    // Pennant: highs slope DOWN, lows slope UP (converging triangle)
    if(rH.slope>=0||rL.slope<=0)continue;
    // Lines must converge by at least 5%
    var spreadStart=Math.max(rH.intercept-rL.intercept,0.001);
    var nReg=regSlice.length-1;
    var spreadEnd=(rH.slope*nReg+rH.intercept)-(rL.slope*nReg+rL.intercept);
    if(spreadEnd<=0||spreadEnd>=spreadStart*0.95)continue;
    // Find the pole: bullish impulse before consolidation (2-20 candles, 2%+ gain)
    var poleFound=false;
    var poleData=null;
    for(var pLen=2;pLen<=20;pLen++){
      var ps=consStart-pLen;
      if(ps<0)break;
      var pSlice=ohlc.slice(ps,consStart+1);
      var poleOpen=pSlice[0].o;
      var poleClose=pSlice[pSlice.length-1].c;
      var gain=(poleClose-poleOpen)/(poleOpen||1);
      if(gain<0.02)continue; // pole must gain at least 2%
      var bullCount=pSlice.filter(function(c){return c.c>c.o;}).length;
      if(bullCount<pLen*0.4)continue; // at least 40% bullish candles
      poleFound=true;
      poleData={start:ps,end:consStart,gainPct:gain,height:poleClose-poleOpen,poleClose:poleClose};
      break;
    }
    if(!poleFound)continue;
    // Project trendlines ONE step beyond regression data to the current (breakout) candle
    var nLast=regSlice.length; // one position beyond last regression point
    var upperNow=rH.slope*nLast+rH.intercept;
    var lowerNow=rL.slope*nLast+rL.intercept;
    var lastC=ohlc[consEnd];
    var curPrice=lastC.c;
    // Breakout: close above extrapolated upper trendline
    var breakout=curPrice>upperNow;
    var forming=!breakout;
    var projBase=breakout?curPrice:upperNow;
    var targetPrice=projBase+poleData.height;
    // False-breakout: weak candle structure (no volume check — unreliable in crypto)
    var falseBrkWarning=false;
    var falseBrkReasons=[];
    if(breakout){
      var body=Math.abs(lastC.c-lastC.o);
      var upperWick=lastC.h-Math.max(lastC.c,lastC.o);
      // Long upper wick (price poked above but closed weak)
      if(body>0&&upperWick>body*1.5){
        falseBrkWarning=true;falseBrkReasons.push("mecha superior muy larga (cierre débil)");
      }
      // Close back inside the triangle
      if(lastC.c<upperNow){
        falseBrkWarning=true;falseBrkReasons.push("cierre aún por debajo de la línea");
      }
    }
    best={
      type:"pennant_bull",
      poleStart:poleData.start,poleEnd:poleData.end,
      consStart:consStart,consEnd:consEnd,
      breakout:breakout,forming:forming,falseBrkWarning:falseBrkWarning,falseBrkReasons:falseBrkReasons,
      targetPrice:targetPrice,upperLine:upperNow,lowerLine:lowerNow,
      poleGainPct:poleData.gainPct,poleHeight:poleData.height,consLength:consLen
    };
    break;
  }
  return best;
...
// Structure: strong downward pole → converging triangle consolidation (descending highs,
// ascending lows, decreasing volume) → bearish breakout below lower trendline with volume
function detectBearishPennant(ohlc){
  if(!ohlc||ohlc.length<20)return null;
  var best=null;
  for(var consLen=15;consLen>=5;consLen--){
    var consEnd=ohlc.length-1;
    var consStart=consEnd-consLen+1;
    if(consStart<5)continue;
    // Build regression excluding the last (potential breakout) candle
    var regSlice=ohlc.slice(consStart,consEnd);
    if(regSlice.length<4)continue;
    var highPts=regSlice.map(function(c,i){return{x:i,y:c.h};});
    var lowPts=regSlice.map(function(c,i){return{x:i,y:c.l};});
    var rH=linReg(highPts);
    ...
    var spreadEnd=(rH.slope*nReg+rH.intercept)-(rL.slope*nReg+rL.intercept);
    if(spreadEnd<=0||spreadEnd>=spreadStart*0.95)continue;
    // Find the pole: bearish impulse before consolidation (2-20 candles, 2%+ drop)
    var poleFound=false;
    var poleData=null;
    for(var pLen=2;pLen<=20;pLen++){
      var ps=consStart-pLen;
      ...
      var poleOpen=pSlice[0].o;
      var poleClose=pSlice[pSlice.length-1].c;
      var drop=(poleOpen-poleClose)/(poleOpen||1);
      if(drop<0.02)continue; // pole must drop at least 2%
      var bearCount=pSlice.filter(function(c){return c.c<c.o;}).length;
      if(bearCount<pLen*0.4)continue; // at least 40% bearish candles
      poleFound=true;
      poleData={start:ps,end:consStart,dropPct:drop,height:poleOpen-poleClose,poleClose:poleClose};
      break;
    }
    if(!poleFound)continue;
    // Project trendlines ONE step beyond regression data to the current candle
    var nLast=regSlice.length;
    var upperNow=rH.slope*nLast+rH.intercept;
    var lowerNow=rL.slope*nLast+rL.intercept;
    var lastC=ohlc[consEnd];
    var curPrice=lastC.c;
    // Breakout: close BELOW extrapolated lower trendline
    var breakout=curPrice<lowerNow;
    var forming=!breakout;
    var projBase=breakout?curPrice:lowerNow;
    var targetPrice=projBase-poleData.height;
    // False-breakout: weak candle structure only (no volume check)
    var falseBrkWarning=false;
    var falseBrkReasons=[];
    if(breakout){
      var body=Math.abs(lastC.c-lastC.o);
      var lowerWick=Math.min(lastC.c,lastC.o)-lastC.l;
      // Long lower wick (price poked below but closed higher — trap)
      if(body>0&&lowerWick>body*1.5){
        falseBrkWarning=true;falseBrkReasons.push("mecha inferior muy larga (cierre débil)");
      }
      // Close back inside the triangle
      if(lastC.c>lowerNow){
        falseBrkWarning=true;falseBrkReasons.push("cierre aún por encima de la línea");
      }
    }
    best={
      type:"pennant_bear",
      ...
      breakout:breakout,forming:forming,falseBrkWarning:falseBrkWarning,falseBrkReasons:falseBrkReasons,
      targetPrice:targetPrice,upperLine:upperNow,lowerLine:lowerNow,
      poleDropPct:poleData.dropPct,poleHeight:poleData.height,consLength:consLen
    };
    break;
  }
  return best;
...
// Retorna null, o un objeto con tipo, posición, breakout y validación de falsa ruptura
function detectChannelAlert(ohlc){
  ...
  var vols=recent.map(function(c){return c.v||0;});
  var price=recent[n].c;

  // Pivot detection with window=2 (2 candles each side — more pivots detected in crypto)
  var pivs=detectPivots(highs,lows,2);
  ...
  // Ascending channel: at least the regression slope must be positive on both lines (already guaranteed by sameSign)
  ...
  // ─── Validación obligatoria: canal requiere al menos 1 toque en cada lado ───
  if(supportTouches<1||resistTouches<1)return null;

  // Posición del precio dentro del canal (0 = soporte, 1 = resistencia)
  var pos=(price-botLine)/ch;

  // Volumen medio últimas 10 velas (para validar ruptura)
  var avgVol10=vols.slice(-10).reduce(function(a,v){return a+v;},0)/10;
  ...
  if(isBreakoutUp||isBreakoutDown){
    ...
    var lastCandle=recent[n];
    var body=Math.abs(lastCandle.c-lastCandle.o);
    var upperWick=lastCandle.h-Math.max(lastCandle.c,lastCandle.o);
    var lowerWick=Math.min(lastCandle.c,lastCandle.o)-lastCandle.l;
    var totalRange=lastCandle.h-lastCandle.l;
    var falseBrkReasons=[];

    if(isBreakoutUp){
      // Ruptura alcista
      if(totalRange>0&&body>0&&upperWick>body*1.5){falseBrkReasons.push("mecha superior muy larga");}
      if(lastCandle.c<topLine){falseBrkReasons.push("cierre dentro del canal");}
    }else{
      // Ruptura bajista
      if(totalRange>0&&body>0&&lowerWick>body*1.5){falseBrkReasons.push("mecha inferior muy larga");}
      if(lastCandle.c>botLine){falseBrkReasons.push("cierre dentro del canal");}
    }
    // False breakout only if BOTH structural signals present (no volume check — unreliable in crypto)
    var falseBrk=falseBrkReasons.length>=2;

    var dir=isBreakoutUp?"alcista":"bajista";
    var alertType=(isBreakoutUp?"canal_"+canalType+"_ruptura_alcista":"canal_"+canalType+"_ruptura_bajista");
    var desc=(isDescending&&isBreakoutUp)
      ?"🔼 Ruptura alcista de canal bajista — posible cambio de tendencia"
  ...
  // ─── Sin ruptura: precio cerca de soporte o resistencia ───
  if(pos<0.18){
...
async function getBinanceData(selAsset,selTF,ASSETS,TFS){
  ...
  var pennantResult=detectBullishPennant(ohlcSeries);
  return{asset:assetLabel,tf:tfLabel,price:price.toFixed(4),change24h:ticker.priceChangePercent,
    ...
    supports:sr.supports,resistances:sr.resistances,fvgs:fvgs,divResult:divResult,pennantResult:pennantResult};
}
